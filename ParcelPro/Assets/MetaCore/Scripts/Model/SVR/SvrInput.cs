...
public class SvrInput : MonoBehaviour
{
    ...
    public enum eInputStyle { Press, Tap, Hold, Release, None }
    ...
    {
        if (svrManager) svrManager.RecenterTracking();
        if (PrimaryController) PrimaryController.Recenter();
        if (SecondaryController) SecondaryController.Recenter();
    }
    ...
    void Update()
    {
        ...
        if (!Input.backButtonLeavesApp &&
            CheckButton(Input.GetKey(KeyCode.Escape)
                || (PrimaryController && PrimaryController.GetButton(SvrController.svrControllerButton.Back)) || (SecondaryController && SecondaryController.GetButton(SvrController.svrControllerButton.Back)),
        ...
        if (CheckButton(Input.GetKey(KeyCode.Return) || Input.GetKey((KeyCode)10) || Input.GetMouseButton(0) // SVR845 Select, Touch, Mouse Left
                || (PrimaryController && (PrimaryController.GetButton(SvrController.svrControllerButton.Start) || PrimaryController.GetButton(SvrController.svrControllerButton.PrimaryShoulder)))
                || (SecondaryController && (SecondaryController.GetButton(SvrController.svrControllerButton.Start) || SecondaryController.GetButton(SvrController.svrControllerButton.PrimaryShoulder))),
    ...
    public bool CheckButton(bool buttonValue, eInputStyle buttonInputType, ref float buttonHoldTimer)
    {
        ...
        {
            if (buttonInputType == eInputStyle.Release)
            {
                buttonHoldTimer = 0; // Terminate timer
                Debug.Log("SvrInput - Release");
                return true;
            }
