...
public class SvrEye : MonoBehaviour, IComparable<SvrEye>
{
    ...
    public enum eType   // Todo: Replace with SvrManager.LayerType
    {
        RenderTexture = 0,
        StandardTexture = 1,
        EglTexture = 2,
    ...
    void InitializeCameras()
    {
        ...
        var frustum = side == SvrPlugin.EyeMask.kRight ? deviceInfo.targetFrustumRight : deviceInfo.targetFrustumLeft;
        ...
        foreach (var mainCamera in mainCameras)
        {
            ...
            else if (frustumType == (int)SvrManager.SvrSettings.eFrustumType.Device)
            {
                ...
                //Debug.LogFormat("PNCDEBUG_ fov :: " + mainCamera.fieldOfView + " aspect :: " + mainCamera.aspect + " left :: " + frustum.left + " right :: " + frustum.right + " bottom :: " + frustum.bottom + " top :: " + frustum.top);
                mainCamera.fieldOfView = 45;
            }

            if (/*SvrPlugin.Instance.Is3drOcclusion() &&*/ imageType == eType.RenderTexture)
            {
                mainCamera.AddCommandBuffer(UnityEngine.Rendering.CameraEvent.BeforeForwardOpaque, SvrPlugin.Instance.beginEyeCommandBuffer);
            }
        }
    }
    ...
    void InitializeCoords()
    {
        ...
        // clipLowerLeft.Set(-1, -1.6f, 0, 1);
        // clipUpperLeft.Set(-1, 1, 0, 1);
        // clipUpperRight.Set(1, 1.6f, 0, 1);
        // clipLowerRight.Set(1, -1, 0, 1);
    }
    ...
    void OnPreRender()
    {
        ...
        if (OnPreRenderListener != null)
        {
            ...
            if (OnOccludeRenderListener != null)
            {
                var viewCamera = mainCameras[0];
                if (viewCamera != null)
                {
                    OnOccludeRenderListener(viewCamera.projectionMatrix, viewCamera.worldToCameraMatrix);
                }
            }
        }
    }
